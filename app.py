...
def load_cloud(uploaded_file):
    """Reads .ply/.pcd/.npz/.csv/.txt -> Nx3 float32 [x,y,z]."""
    ...
    if name.endswith(".ply"):
        if not HAS_PLY:
            st.error("Install `plyfile`. It is listed in requirements.txt."); st.stop()
        f = io.BytesIO(buf)
        ply = PlyData.read(f)
        v = ply["vertex"].data
    ...
    if name.endswith(".pcd"):
        if HAS_PCD:
            # pypcd handles ASCII and most binary; compressed may fail
            ...
            try:
                ...
                # try naive ASCII fallback
                try:
                    return read_pcd_ascii(io.StringIO(buf.decode("utf-8")))
                except Exception:
                    st.error(f"PCD read failed: {e}")
                    st.stop()
        else:
            # naive ASCII fallback
            try:
                return read_pcd_ascii(io.StringIO(buf.decode("utf-8")))
            except Exception as e:
                st.error(f"PCD reader not available. Install `pypcd`. Error: {e}")
                st.stop()
    ...
    if name.endswith((".csv", ".txt")):
        ...
        for r in csv.reader(io.StringIO(s)):
            ...
            try:
                ...
                continue
        if not rows:
            st.error("CSV/TXT must have x,y,z columns."); st.stop()
        return np.asarray(rows, dtype=np.float32)

    st.error("Use .ply, .pcd, .npz, .csv, or .txt."); st.stop()

def read_pcd_ascii(f):
    """Minimal ASCII .pcd parser for x y z only."""
    header = []
    while True:
        line = f.readline()
        if not line:
    ...
    idx_x = fields.index("x"); idx_y = fields.index("y"); idx_z = fields.index("z")
    pts = []
    for line in f:
...
# ---------- Demo generators ----------
def rect_outline(cx, cy, w, h, zmin=0.1, zmax=2.4, n=800, noise=0.005, rng=None):
...
def add_door_gap(points, cx, cy, w, h, side="S", gap=0.9, rng=None):
    """Remove points near a mid-wall span to simulate a door gap."""
    rng = rng or np.random.RandomState(0)
    door_center = {
        "S": (cx, cy - h/2),
        "N": (cx, cy + h/2),
    ...
    dcx, dcy = door_center
    # remove within ~gap width along the wall direction
    rad = gap / 2.0
    keep = ~(
        (np.abs(points[:,0] - dcx) < (side in ["S","N"]) * rad + (side in ["E","W"]) * 0.12) &
        (np.abs(points[:,1] - dcy) < (side in ["E","W"]) * rad + (side in ["S","N"]) * 0.12)
    )
    return points[keep]

def demo_scene(name="Studio", seed=42):
    ...
    if name == "Studio":
        ...
        R = add_door_gap(R, cx, cy, w, h, side="S", gap=0.9, rng=rng)
        clouds.append(R)
    elif name == "2BHK":
        # two rooms + living + corridor
        specs = [
            (-2.5, 0.0, 4.5, 3.8, "S"),
            ( 2.5, 0.0, 4.5, 3.8, "E"),
            ( 0.0, 3.5, 10.0, 4.0, "N"),
        ...
        for (cx,cy,w,h,door_side) in specs:
            R = rect_outline(cx, cy, w, h, rng=rng, noise=0.007)
            R = add_door_gap(R, cx, cy, w, h, side=door_side, gap=0.9, rng=rng)
            clouds.append(R)
    elif name == "Corridor network":
        specs = [
            (0.0, 0.0, 16.0, 2.5, "E"),
            (0.0, 4.0, 16.0, 2.5, "W"),
            (-7.5, 2.0, 2.5, 6.0, "S"),
        ...
        for (cx,cy,w,h,door_side) in specs:
            ...
            R = add_door_gap(R, cx, cy, w, h, side=door_side, gap=1.0, rng=rng)
            clouds.append(R)
...
def merge_collinear(lines, angle_tol_deg=5):
    ...
    for i,l in enumerate(lines):
        ...
        if abs(math.sin(math.radians(ag))) < 0.5:  # ~horizontal
...
with st.sidebar:
    ...
    voxel = st.slider("Voxel (m)", 0.01, 0.10, 0.03, 0.01)
    density_cell = st.slider("Outlier cell (m)", 0.03, 0.20, 0.06, 0.01)
    density_min = st.slider("Min pts per cell", 1, 12, 3, 1)
    zmin = st.number_input("z_min (m)", 0.0, 1.0, 0.10, 0.05)
    zmax = st.number_input("z_max (m)", 1.5, 4.0, 2.40, 0.05)

    st.header("Grid / Vectorization")
    res = st.slider("Grid resolution (m/px)", 0.01, 0.10, DEFAULT_RES_M, 0.01)
...
if run:
    with st.spinner("Preparing data"):
        if mode == "Upload":
            ...
            pts = demo_scene(demo_name, seed=int(demo_seed))

        if pts.ndim != 2 or pts.shape[1] < 3:
            st.error("Expect Nx3 points [x,y,z]."); st.stop()

        orig_n = pts.shape[0]
        if orig_n > MAX_POINTS:
            step = int(np.ceil(orig_n / MAX_POINTS))
            pts = pts[::step]
    ...
    with col1:
        ...
        samp = pts[np.random.choice(len(pts), size=min(SAMPLE_FOR_VIEW, len(pts)), replace=False)]
        ...
        st.caption(f"Loaded: {orig_n:,} pts → kept: {pts.shape[0]:,}. Band: {band.shape[0]:,}.")
